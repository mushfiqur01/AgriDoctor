...
import { PredictionResult as PredictionResultType, CropType } from '@/services/predictionService';
import { CheckCircle2, AlertCircle, Leaf, Home, RefreshCcw } from 'lucide-react';
...
export function PredictionResultScreen({ imageData, result, onScanAgain, onGoHome }: PredictionResultProps) {
  const { t, language } = useLanguage();
  
  // Check if healthy by looking for "Healthy" in the label
  ...
  const statusBg = isHealthy ? 'bg-accent/10' : 'bg-destructive/10';
    ...
    <div className="min-h-screen bg-soft-gradient">
      ...
      <main className="pt-20 pb-8 px-4">
        <div className="max-w-lg mx-auto">
          ...
          {/* Status Badge */}
          <div className={`flex items-center justify-center gap-2 px-4 py-3 rounded-2xl ${statusBg} mb-6 animate-scale-in`} style={{ animationDelay: '100ms' }}>
            <StatusIcon className={`w-6 h-6 ${statusColor}`} />
            <span className={`font-bold text-lg ${statusColor} ${language === 'bn' ? 'font-bangla' : ''}`}>
          ...
          <div className="space-y-4 mb-8">
            {/* Crop Info */}
            <div className="result-card animate-slide-up" style={{ animationDelay: '150ms' }}>
              <div className="flex items-center gap-3">
                <span className="text-3xl">{cropEmojis[result.crop]}</span>
                <div>
            ...
            <div className="result-card animate-slide-up" style={{ animationDelay: '200ms' }}>
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className={`text-sm text-muted-foreground mb-1 ${language === 'bn' ? 'font-bangla' : ''}`}>
                    {t('prediction.description')}
                  </p>
                  <p className={`text-foreground leading-relaxed ${language === 'bn' ? 'font-bangla' : ''}`}>
            ...
            {/* Solutions */}
            <div className="result-card animate-slide-up" style={{ animationDelay: '250ms' }}>
              <p className={`text-sm text-muted-foreground mb-3 ${language === 'bn' ? 'font-bangla' : ''}`}>
                {t('prediction.solutions')}
              </p>
              <div className="space-y-3">
                {result.diseaseInfo.solutions.map((solution, index) => (
                  <div key={index} className="solution-item">
                    <div className="w-6 h-6 rounded-full bg-primary/20 flex items-center justify-center flex-shrink-0">
                      <span className="text-xs font-bold text-primary">{index + 1}</span>
                    </div>
                    <p className={`text-foreground ${language === 'bn' ? 'font-bangla' : ''}`}>
                      {solution}
                    </p>
                  </div>
                ))}
